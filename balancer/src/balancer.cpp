#include <balancer.hpp>
  ...
  ACTION balancer::createdrop(int dropnum, int changeprice,int changepricetime, string format, name username, name collection, name shemas, uint32_t templates, asset price, int supply, uint64_t dropstart, uint64_t dropend, string img,string drop_name, string description){
...
ACTION balancer::newuser(name username){
  require_auth(username);
...
// slippage
ACTION balancer::setslip(name username, float slip){
  require_auth(username);
...
void balancer::token_transfer(name from,name to, asset quantity, string memo){
  // auth user
  require_auth(from);
...
//mint
void balancer::in_contract_claim(name username, string memo, asset quantity){
  // memo(int)
...
// transfer tokens
void balancer::in_contract_transfer(name username, asset quantity){
  string memo;
  action{
    permission_level{get_self(), "active"_n},
    "eosio.token"_n,
...
//set drop time
ACTION balancer::setdroptime(int drop_id, uint64_t dropstart, uint64_t dropend){
  auto itr_drop = drop_table.find(drop_id);
  require_auth(itr_drop->username);
...
//set price
ACTION balancer::setprice(int drop_id, asset price){
  auto itr_drop = drop_table.find(drop_id);
  require_auth(itr_drop->username);
  drop_table.modify(itr_drop, get_self(), [&](auto& row) {
    row.startprice = asset{price.amount, price.symbol};
...
// set drop info
ACTION balancer::setinfo(int drop_id, string img, string drop_name, string description){
  auto itr_drop = drop_table.find(drop_id);
  require_auth(itr_drop->username);
  drop_table.modify(itr_drop, get_self(), [&](auto& row) {
    row.img = img;
...
// set drop supply
ACTION balancer::setsupply( int drop_id, int supply){
  auto itr_drop = drop_table.find(drop_id);
  require_auth(itr_drop->username);
  drop_table.modify(itr_drop, get_self(), [&](auto& row) {
    row.maxsupply = supply;
...
// set change prices
ACTION balancer::setchangepr( int drop_id, int changeprice,int changepricetime){
  auto itr_drop = drop_table.find(drop_id);
  require_auth(itr_drop->username);
  drop_table.modify(itr_drop, get_self(), [&](auto& row) {
    row.changeprice = changeprice;
